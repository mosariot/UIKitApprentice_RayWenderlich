...
//  MyLocations
//
//  Created by Александр Воробьев on 03.05.2021.
//
                ...
                if self.isViewLoaded {
                    if let dictionary = notification.userInfo {
                        if let insertedLocations = dictionary[NSInsertedObjectsKey] as? Set<Location> {
                            let locations = Array(insertedLocations)
                        ...
                        if let updatedLocations = dictionary[NSUpdatedObjectsKey] as? Set<Location> {
                            let locations = Array(updatedLocations)
                            for location in locations {
                                self.locations[self.locations.firstIndex(of: location)!] = location
                        ...
                        if let deletedLocations = dictionary[NSDeletedObjectsKey] as? Set<Location> {
                            let locations = Array(deletedLocations)
                            for location in locations {
                                self.mapView.removeAnnotation(location)
                                self.locations.remove(at: self.locations.firstIndex(of: location)!)
                            }
...
extension MapViewController: MKMapViewDelegate {
    ...
    func mapView(_ mapView: MKMapView, viewFor annotation: MKAnnotation) -> MKAnnotationView? {
        ...
        if annotationView == nil {
            ...
            pinView.pinTintColor = UIColor(red: 0.32, green: 0.82, blue: 0.40, alpha: 1)
